...
## WHATS COMING - FEATURES ENHANCEMENTS - REAL PRODUCT WORTHINESS X10
...
RAG Pipeline:
User asks: "What anomalies near Frankfurt in the last 24h?"
Embed query via MiniLM (browser or server).
Retrieve top-K chunks from ChromaDB (detections, news, alerts).
Construct prompt with retrieved context + system instructions.
Generate structured JSON response with assumptions, correlations, predictions.
3.3 Autonomous Correlation Engine
Runs on a schedule (every 5 minutes) or triggered by new data:

Spatial correlation: Events within X km of each other.
Temporal correlation: Events within Y hours of each other.
Semantic correlation: Similar embeddings (cosine similarity > 0.85).
Causal inference: LLM judges if Event A likely influences Event B.
...
Data Provenance: Every ingested event stores its source URL, fetch time, and license.
Privacy by Design: No individual tracking. Aggregate-only for sensitive feeds (crime, registries).
Rate Limiting: Per-IP and per-API-key limits on all endpoints.
Audit Logging: All AI assistant queries and alert dismissals are logged.
Content Filtering: LLM outputs are scanned for harmful content before display.
